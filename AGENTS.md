...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/gpg`](https://github.com/genzouw/gpg) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は、5節の例外申請プロセスに基づく事前承認がある場合を除き **MUST NOT** で、提出された場合は内容の良し悪しにかかわらず自動的にクローズされます。

### 1.1 MUST NOT — 原則としてこれらを含む PR は自動的にクローズされます（例外は5節を参照）
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **MUST** 取得してください。承認のない有料サービス導入 PR は自動的にクローズされます。
Issue では「なぜ既存の無料サービスでは目的を達成できないか」「課金リスクをどう管理するか」を明確に書いてください。

承認済みの例外として PR を提出する場合は、承認済み Issue への URL を PR 本文に **MUST** 明記してください。リンクが無い、またはオーナーの明示的な承認が確認できない PR は、事前承認済みの例外とはみなされず、1 節 Line 20 の規定に従い自動的にクローズされます。
...
本ポリシーは公開リポジトリ [`genzouw/gpg`](https://github.com/genzouw/gpg) に適用されます。他の公開リポジトリへ本ファイルを伝播する仕組みは現時点で存在しないため、他リポジトリで同様の方針を適用したい場合は、当該リポジトリに個別に `AGENTS.md` を配置してください。
